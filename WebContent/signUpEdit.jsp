...
<html>
	...
	<body data-spy="scroll" data-target=".main-nav">
		...
		<!--여기서부터 본문 작성하면 됨! -->
        <section class="contact" id="signUpEdit">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
				...
				<div class="row" style="display:flex; justify-content: center;">
					<div class="col-md-4">
						<form action="./MemberUpdateAction.me?email=<%=member.getEmail() %>" class="form-group" name="newMember" onsubmit="return checkForm()">
							<label for="name">이름/별명</label>
							<input type="text" id="name" class="form-control" name="name" value="<%=member.getName() %>">
							<label for="email">이메일 (가입 이후에 변경이 불가합니다)</label> 
							<input type="email" id="email" class="form-control" name="email" value="<%=member.getEmail() %>" readOnly>
							...
							<input type="text" id="phonenumber" name="phonenumber" class="form-control" value="<%=member.getPhonenumber() %>"">
							<div style="float: right">
								<input type="submit" class="btn btn-main btn-md" value="수정">
							</div>
							<div style="float: left">
								<a href=./MemberDeleteAction.me?email=<%=member.getEmail() %> class="btn btn-main btn-md">회원 탈퇴</a>
							</div>
							<div style="float: center">
								<a href="./welcome.jsp" class="btn btn-main btn-md" >취소</a>
							</div>
						</form>
					</div>
				</div>
